...
#define MAXDISTANCE 0.25
#define ROTDEGREE toRadian(20)
...
int main(int argc, char **argv)
{
	...
	while (ros::ok()) {
		...
		avg /= nRangeSize;
		//여기서 장애물과의 거리를 프린트한다.
		printf("##############AVG DISTANCE : %lf############\n", avg);
		//어느 이상 가까이 오면
		//MAXDISTANCE 는 0.25정도로 define 해 놓았음.
		double deg;
		if (avg < MAXDISTANCE) {
			//console 에 장애물 발견!
			//회전각 세타의 +, - 를 랜덤하게 정하기.
			//ROTDEGREE 는 로테이션할 각도 20도를 라디안으로 변환시켜놓은 값으로 위에서 define 해 두었음.
			srand(time(NULL));
			//rand() %2 는 0,1
			if((rand()%2) == 1)
				deg = ROTDEGREE;
