...
int main(int argc, char **argv)
{
    ...
    while(ros::ok()) {
		...
		//Average distance getting

		int nRangeSize = (int)laserScanXY.size();
		double avg = 0;
		for(int i=0; i<nRangeSize; i++) {
			double x_temp = laserScanXY[i][0];
			double y_temp = laserScanXY[i][1];
			printf("############## x: %lf y: %lf#################\n", x_temp, y_temp);
			if(isnan(x_temp) || isnan(y_temp)) // nan check
				continue;
			else if( (-5 < x_temp && x_temp <5) && (-5 < y_temp && y_temp <5)) // inf check
				avg += sqrt(pow(laserScanXY[i][0],2.0) + pow(laserScanXY[i][1],2.0));
    	}
		avg /= nRangeSize;
